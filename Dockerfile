...
RUN apt-get update && apt-get install -y  git-core imagemagick libcurl4-openssl-dev libgit2-dev libicu-dev libmagic-dev libmagick++-dev libpng-dev libssl-dev libxml2-dev make pandoc zlib1g-dev && rm -rf /var/lib/apt/lists/*
...
RUN Rscript -e 'remotes::install_version("gtExtras",upgrade="never", version = "0.4.2")'
...
RUN Rscript -e 'remotes::install_version("forcats",upgrade="never", version = "0.5.2")'
...
EXPOSE 80
CMD R -e "options('shiny.port'=80,shiny.host='0.0.0.0');recruit.athleticism::run_app()"
