...
athleticism_table <- function(input_df, input_player) {
  ...
  df <- input_df |>
    ...
    dplyr::ungroup() |>
    dplyr::mutate(Comp = (.data$Comp - min(.data$Comp, na.rm = TRUE)) / (max(.data$Comp, na.rm = TRUE) - min(.data$Comp, na.rm = TRUE)),
                  Ath = .data$`ATH%`) |>
    ...
    dplyr::filter(.data$Similarity != max(.data$Similarity), !is.na(.data$player)) |>
